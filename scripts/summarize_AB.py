...
def main() -> int:
    ...
    print(summary.to_string(index=False, float_format=lambda x: f'{x:.5g}'))

    for metric in ('f1', 'chamfer', 'hausdorff'):
        print(f'\n=== Mean {metric} by (combo, grid_len) — successful cells only ===')
        piv = (df[df['_real']].groupby(['algo', 'grid_len'])[metric]
               .mean().unstack())
        print(piv.to_string(float_format=lambda x: f'{x:.6g}'))
